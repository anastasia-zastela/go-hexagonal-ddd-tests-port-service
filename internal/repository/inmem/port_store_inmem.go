...
// !!!! added later by me
// func (s *PortStore) bulkCreateOrUpdatePorts(ctx context.Context, tx *sql.Tx, ports []*store.Port) error {
// 	if len(ports) == 0 {
// 		return nil
// 	}
...
// 	valueArgs := make([]interface{}, 0, len(ports)*4) // Adjust the multiplier based on the number of columns
// 	for _, p := range ports {
// 		valueStrings = append(valueStrings, "(?, ?, ?, ?)")            // Adjust based on the number of columns
// 		valueArgs = append(valueArgs, p.ID, p.Name, p.City, p.Country) // Include all fields of the port
// 	}
